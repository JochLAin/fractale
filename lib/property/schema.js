...
class Schema {
    constructor(entity, props = {}) {
        ...
        if (!this.entity.parent) this.values = this.fields;
        else this.values = [].concat(this.entity.parent.schema.values, this.fields);
    ...
    get(key, full = true) {
        ...
        const field = fields.filter(field => !field.ƒ_regexp).find(field => field.is(key));
        if (field) return field;
        return fields.filter(field => field.ƒ_regexp).find(field => field.is(key));
    }
