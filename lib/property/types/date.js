...
module.exports = class ƒ_date extends ƒ_type {
    ...
    static evaluate(input, type) {
        return type === 'string' && ['date'].includes(input)
            ...
            || type === 'object' && (input instanceof Date || (moment && input instanceof moment))
        ;
    }
    ...
    validate(value) {
        ...
        if (typeof this.validator === 'object') {
            ...
            if (Object.prototype.hasOwnProperty.call(this.validator, 'between')) {
                if (!DateUtils.between(value, this.validator.between[0], this.validator.between[1] || new Date())) {
                    throw this.createValidatorError('between', DateUtils.display(value), `between ${DateUtils.display(this.validator.between[0])} and ${DateUtils.display(this.validator.between[1] || new Date())}`);
                }
            }
