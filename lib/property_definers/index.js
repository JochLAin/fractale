    ...
    for (let key in schema) {
        if (schema.hasOwnProperty(key)) {
            ...
            if (params[key] !== undefined && params[key] !== null) {
                // console.log(instance.constructor.name, key, params[key], definer.normalize(definer.check(params[key])));
    ...
    for (let key in schema) {
        if (schema.hasOwnProperty(key)) {
            serialized[key] = Array.isArray(schema[key])
                ? instance.props[key].map(value => single(value))
                : single(instance.props[key])
            ;
        }
    }
