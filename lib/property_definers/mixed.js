...
MixedPropertyDefiner.createUncorrectTypeError = (definer, value, type) => {
    if (value instanceof ModelClass) {
        return new UncorrectTypeError(definer,`${definer.type.name}.${definer.key}`, type, value.constructor.name);
    } else {
        return new UncorrectTypeError(definer,`${definer.name}.${definer.key}`, type, typeof value);
    }
};

MixedPropertyDefiner.createUncorrectItemTypeError = (error, key) => {
    console.log(error);
    return new UncorrectTypeError(error.definer, `${error.definer.name}.${key}`, error.type, error.value, true);
};
