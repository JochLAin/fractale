    ...
    getter() {
        return () => {
            return (id) => {
                ...
                if (index >= 0) {
                    const value = this.denormalize(this.instance.props[`_${this.key}`][index]);
                    const previous = value;
                    if (value && value.addEventListener instanceof Function) {
                        value.addEventListener('change', () => this.instance.dispatchEvent('change', {
                            key: Array.isArray(this.singular) ? this.singular[0] : this.singular,
                            value: this.value,
                            previous
                        }));
                    }
