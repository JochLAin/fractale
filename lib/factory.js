    ...
    return class Entity extends Parent {
        ...
        static stringify(space) {
            const schema = JSON.stringify(Entity.schema, (k, v) => v || 'undefined', space).replace(/("([a-zA-Z0-9\-_.()']+)":?)/g, (match, content, value) => {
                ...
                return content;
            }).replace(/"Fractale\.with\(([a-zA-Z0-9\-_.()']+), (.+)\)"/g, (match, type, options) => {
                return `Fractale.with(${type}, ${_.stringify(_.parse(options.replace(/\\"/g, '"')))})`
            }).replace(/"(\w+)n"/g, `$1n`);
        ...
        static getDependencies(dependencies = {}) {
            const closure = (accu, type) => {
                ...
                if (type.generator.name === type.schema.classname) return accu;
                if (type.generator.virtual) return accu;
                if (accu[type.generator.name]) return accu;
                Object.assign(accu, { [type.generator.name]: type.generator });
                Object.assign(accu, type.generator.getDependencies(accu));
                return accu;
            };
