            ...
            constructor(props = {}, overrider) {
                ...
                for (let key in schema) {
                    if (schema.hasOwnProperty(key)) {
                        ...
                        definer.assign(this.props, key, schema[key]);
                        definer.define(this.props, key, schema[key]);
                        if (props[key]) {
                            this[key] = props[key];
                        }
                    }
                ...
                this.addEventListener('change', ({ key, value }) => {
                    const promise = encyclopedia.memory.update(this.serialize(false));
                ...
                for (let key in schema) {
                    if (schema.hasOwnProperty(key)) {
                        if (Array.isArray(schema[key])) {
                            serialized[key] = this[key].map(value => single(this, schema, key, value));
                        } else {
                            serialized[key] = single(this, schema, key, this[key]);
                        }
                    }
                }
