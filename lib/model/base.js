...
module.exports = class Model extends EventListener {
    ...
    get(name) {
        ...
        return this.ƒ_properties.get(field.key).get(name);
    }

    set(name, value) {
        const field = this.constructor.schema.get(name);
        this.ƒ_properties.get(field.key).set(value, name);
        return this;
    }
    ...
    deserialize(data) {
        for (const key in data) {
            ...
            if (this.constructor.schema.has(key)) {
                this.set(key, data[key], key);
            } else {
    ...
    static getDependencies(model, dependencies = {}) {
        const closure = (accu, field) => {
            if (field.ƒ_array) return closure(accu, field.fork())
